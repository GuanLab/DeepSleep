...
if __name__ == '__main__':
    ...
    for filename in all_test_files:
        ...
        image_batch=[]
        for k in np.arange(0,65280,480):
            image_batch.append(image[:,k:(k+480)].T)
...
#        output = model0.predict(input_pred)
        ...
        output_new=np.reshape(output,(1360))
        #output_new=np.reshape(output,(size*batch))

        #print(output_new)
        output_part1=np.repeat(output_new[np.arange(0,1360,2)],2**8*48)
        output_part2=np.repeat(output_new[np.arange(1,1360,2)],2**8*48)        

        # deal with half-overlapping
        output_final=np.zeros(65280*128+128)
        output_count=np.zeros(65280*128+128)
        output_final[:65280*128] = output_final[:65280*128] + output_part1
        output_count[:65280*128] = output_count[:65280*128] + 1
