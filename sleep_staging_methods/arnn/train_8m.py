...
####################################
    ...
    while True:
        ...
        for b in range(batch_size):
            ...
            tmp=np.concatenate((tmp[1:],tmp[:1]))
            label_final=label_major_vote(tmp.reshape((1,len(tmp))),2)

            index=np.arange(0,65520,48)
            random.shuffle(index)
            for k in index: # 48 -> 30-second
                if np.sum(label_final[:,k:(k+48)]!=-1) > 0:
                    image_batch.append(image_final[:,k:(k+48)].T)
                    label_batch.append(label_final[:,k:(k+48)].T)
